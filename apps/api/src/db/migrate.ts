...
CREATE TABLE IF NOT EXISTS users (
  ...
  tier          VARCHAR(16) NOT NULL DEFAULT 'free'
                CHECK (tier IN ('free','pro','teams','creator')),
...
CREATE TABLE IF NOT EXISTS boards (
  ...
  category           VARCHAR(32) NOT NULL DEFAULT 'custom'
                     CHECK (category IN ('sales','gaming','fitness','music','sports','custom')),
  scoring_type       VARCHAR(32) NOT NULL DEFAULT 'highest'
                     CHECK (scoring_type IN ('highest','lowest','cumulative','streak','head_to_head')),
...
CREATE TABLE IF NOT EXISTS board_memberships (
  ...
  role       VARCHAR(16) NOT NULL DEFAULT 'member'
             CHECK (role IN ('owner','admin','member','spectator')),
...
CREATE TABLE IF NOT EXISTS scores (
  ...
  source              VARCHAR(32) NOT NULL DEFAULT 'manual'
                      CHECK (source IN ('manual','integration','webhook','sensor')),
...
-- ─── Rank Snapshots (for trajectory graphs & Coach AI) ───────────────────────
...
CREATE INDEX IF NOT EXISTS idx_snapshots_board_user_time
  ON rank_snapshots(board_id, user_id, recorded_at DESC);
